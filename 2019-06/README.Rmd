---
title: "2019 (June) Democratic Debates data project"
author: "Jane Doe"
...
### Candidates with medium and high polling criterions (last seven days)

We will repeat the graph above, but with a lower range of days. We will also add the `2-4.0% of voters` category.
...
Now we can see Warren is definitely ahead of O'Rourke, but also shows Booker has also moved above Warren. 

### Candidates with low polling criterions (last seven days)
...
GtrendWikiIOTAirTime7day %>% 
  dplyr::filter(poll_perc_fct %in% c("1 - 1.3% of voters")) %>% 
  ggplot2::ggplot(aes(x = date, 
             y = hits, 
             color = keyword)) +
  ggplot2::geom_line(aes(group = keyword), show.legend = FALSE) + 
...
GtrendWikiIOTAirTime7day %>% 
  ...
  ggplot2::geom_line(aes(group = keyword), show.legend = FALSE) + 
  ggplot2::labs(
   subtitle = "Women Democratic Candidates June 26, 2019 Democratic candidates",
    caption = paste0("Data between ", 
                   min(GtrendWikiIOTAirTime7day$date),
                   " and ",
                   max(GtrendWikiIOTAirTime7day$date))) + 
  ggthemes::theme_fivethirtyeight(base_size = 9.5) + 
...
GtrendWikiIOTAirTime7day %>% 
  ...
  ggplot2::labs(
    x = "Date",
    y = "Google search hits",
    subtitle = "Google search trends for Men candidates \nJune 26, 2019 Democratic candidates",
